...
@click.group()
@click.option("--base_url", default="https://api.openai.com")
...
@click.option("--stream", default=True, show_default=True, type=bool)
@coro
...
@click.option(
    ...
    show_default=True,
    type=click.Choice(["text-davinci-003"]),
    show_choices=True,
)
@click.option("--max_tokens", default=1024, show_default=True)
@click.option("--temperature", default=0.5, show_default=True)
@click.option("--stream", default=True, show_default=True, type=bool)
@coro
async def complete(
    prompt: str, model: str, max_tokens: int, temperature: float, stream: bool
):
